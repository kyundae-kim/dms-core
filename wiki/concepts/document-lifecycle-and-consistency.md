...
DMS SRS는 업로드와 삭제를 단순 CRUD 호출이 아니라 원문 저장소와 메타데이터 저장소 사이의 일관성 문제로 정의한다. 2026-06-18 갱신본은 이 정책을 미래형 설명이 아니라 현재 구현이 충족해야 하는 요구사항으로 정리하며, 같은 날짜의 SDK interface 문서는 checksum 자동 계산, object-first upload, `deleting` 선반영, `failed`/`ConsistencyError` 분기까지 포함한 구체적 semantics를 명시한다.^[raw/articles/dms-sdk-interface-2026-06-15.md]
...
## SRS 현재형 전환의 의미
- 이 페이지는 “필요한 정책” 설명을 넘어서 현재 코드가 유지해야 하는 삭제/복구 의미론을 담는다.
- 문서와 구현이 함께 바뀌어야 하므로 README, SDK interface, query 페이지에서도 동일한 partial-failure semantics를 유지해야 한다.
