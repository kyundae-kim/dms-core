...
v0.5.0 공개 API는 일반 애플리케이션의 서비스 lifecycle을 개별 factory 호출보다 assembly API로 먼저 표현하도록 안내한다. 동기 서비스만 조립할 때는 `assemble_services()`를, NATS를 포함하거나 sync·async lifecycle을 함께 다뤄야 할 때는 typed `RuntimePlan`을 전달한 `await assemble_service_runtime(plan=...)`을 사용한다.^[raw/articles/docmesh-py-core-api-reference-v0-4-0.md]

## 조립 계약

두 API는 환경 mapping 로딩, 사용 가능한 서비스 탐지, 선택·필수성·대안 그룹 검증, client 생성, 선택적 startup healthcheck를 한 흐름에 결합한다. `ServiceBundle`은 동기 context manager이고, `ServiceRuntime`은 `Service` enum key를 쓰는 async context manager다. typed runtime에서 `require()`는 미선택과 미초기화를 서로 다른 조회 오류로 구분한다.^[raw/articles/docmesh-py-core-api-reference-v0-4-0.md]
...
개별 `*Config()`과 `create_*_client()` factory는 CLI, 배치, 단일 서비스 검증, SDK hook 또는 client lifecycle을 명시적으로 제어해야 하는 경우에 적합하다. v0.5.0 예제는 동기 direct API에도 `try`/`finally` cleanup을 포함하고, 동기 lifecycle 전체를 소유할 때는 `ServiceBundle` context manager를 사용한다. `assemble_services()`에 NATS가 포함되면 오류가 발생하므로 NATS가 있으면 async runtime을 사용한다. 소비 애플리케이션의 기본 bootstrap 경로는 [[docmesh-py-core]]가 제공하는 assembly-first 모델이며, 새 코드는 [[runtime-planning-and-environment-diagnosis]]의 typed plan을 사용해야 한다.^[raw/articles/docmesh-py-core-examples-v0-4-0.md]
