...
v0.2.0 예제 문서는 FastAPI lifespan 훅에서 `assemble_services()`로 설정 탐색, 필수 서비스 검증, client 생성, startup healthcheck를 한 번에 수행하고, context manager가 종료 cleanup을 맡기는 패턴을 제시한다. NATS 또는 async lifecycle이 필요하면 `await assemble_service_runtime()`과 async context manager를 사용한다.^[raw/articles/docmesh-py-core-examples-v0-1-4.md]
...
- NATS를 포함하면 `async with runtime:`과 `runtime.require("nats")`를 사용한다.^[raw/articles/docmesh-py-core-examples-v0-1-4.md]
