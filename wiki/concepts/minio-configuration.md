...
`docmesh-py-core`는 MinIO 연결에 대해 endpoint, access key, secret key를 필수 축으로 두고, HTTPS 사용 여부와 region, bucket, retry/timeout 정책을 별도 환경변수로 관리한다. v0.4.0 Configuration Guide는 `MINIO_BUCKET`을 application이 `require_minio_bucket()`으로 선택적으로 요구하는 값으로 정의하고, `MINIO_SECURE`은 production에서 반드시 true여야 한다고 명시한다.^[raw/articles/docmesh-py-core-configuration-v0-4-0.md]
...
- 운영 환경에서는 `MINIO_SECURE=false`를 허용하지 않으며, endpoint placeholder도 설정 진단에서 거부한다.^[raw/articles/docmesh-py-core-configuration-v0-4-0.md]
