...
sources: [raw/articles/docmesh-py-core-api-v0-1-1.md, raw/articles/docmesh-py-core-sdk-v0-1-1.md, raw/articles/docmesh-py-core-examples-v0-1-4.md, raw/articles/docmesh-py-core-examples-v0-4-0.md, raw/articles/docmesh-py-core-configuration-v0-4-0.md]
confidence: medium
---

# NatsConnectionBuilder

`NatsConnectionBuilder`는 NATS 연결 정보를 보관하고, 실제 연결 생성과 점검을 비동기적으로 수행하는 builder다. 최신 API 문서에서도 다른 서비스가 `ServiceClientWrapper`를 돌려주는 것과 달리, NATS는 `create_nats_client()`가 이 builder를 반환하는 예외 경로로 유지된다. v0.5.0 설정은 servers, 인증 방식, connect timeout, reconnect, ping/no-echo 옵션을 builder가 보존하도록 명시한다.^[raw/articles/docmesh-py-core-api-v0-1-1.md]^[raw/articles/docmesh-py-core-configuration-v0-4-0.md]
...
- v0.5.0 예제는 `builder.check()`가 임시 연결을 flush한 뒤 닫는 반면, `builder.connect()`가 반환한 지속 연결은 publish/flush 뒤 호출자가 `drain()`해야 함을 분리해 제시한다. `ServiceRuntime.close()`도 이 별도 지속 연결을 대신 닫지 않는다.^[raw/articles/docmesh-py-core-examples-v0-4-0.md]
