...
SDK interface 초안은 `create_sdk(env)` 같은 팩토리 진입점을 통해 소비 프로젝트가 초기화 세부사항을 숨길 수 있어야 한다고 제안한다. 이 팩토리는 `docmesh-py-core.load_settings(env)` 호출, `ServiceFactoryRegistry(settings)` 생성, metadata store와 object store 구현체 조립, startup 시 필수 의존성 health check 수행, 최종 `DocumentManagementSDK` 구현체 반환을 책임진다.^[raw/articles/dms-sdk-interface-2026-06-15.md]
