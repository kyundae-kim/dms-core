...
비동기 lifecycle에는 `async_check_all_services()`가 동기·awaitable check를 함께 실행하며, 개별 및 전체 timeout을 지원한다. required 실패 시 예외의 결과와 실패 목록에서 전체 상태를 확인할 수 있다. `assemble_service_runtime()`은 이 healthcheck와 생성 실패 rollback을 runtime lifecycle에 통합한다.^[raw/articles/docmesh-py-core-api-v0-1-1.md]
