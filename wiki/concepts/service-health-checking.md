...
설정 문서에는 `DOCMESH_HEALTHCHECK_ENABLED`가 공통 토글로 정의되어 있으며, 각 서비스는 timeout/retry를 공통값이 아니라 서비스별 환경변수로 가진다. 또한 운영 환경에서는 `KEYCLOAK_VERIFY_SSL`, `MINIO_SECURE`, `MILVUS_SECURE`에 대한 추가 보안 제약이 활성화되므로, 헬스체크는 단순 on/off 기능이 아니라 서비스별 연결 특성과 운영 보안 정책을 함께 반영하는 설정 계약과 연결된다.^[raw/articles/docmesh-py-core-config-v0-1-1.md]
