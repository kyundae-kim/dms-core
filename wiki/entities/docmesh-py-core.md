...
- 설정은 모두 프로세스 환경변수 기반이며, 서비스별 timeout/retry/security 규칙을 별도 관리한다. `DOCMESH_ENV`가 `prod`/`production`이거나 `DOCMESH_SECURITY_MODE=production`이면 transport security와 placeholder 값 검증이 적용된다. `DOCMESH_LOG_LEVEL`처럼 공통 config 객체가 아니라 로깅 초기화 함수가 직접 읽는 환경변수도 존재한다.^[raw/articles/docmesh-py-core-configuration-v0-4-0.md]
- 소비 프로젝트용 최신 public lifecycle은 설정 mapping 준비 → `RuntimePlan`과 `HealthcheckPolicy` 선언 → `await assemble_service_runtime(plan=...)` → async context manager 기반 종료 흐름으로 정리된다. v0.3.0 문서는 문자열 기반 선택 인자의 제거 목표를 v0.4.0으로 제시했으며, 현재 v0.4.0 공개 레퍼런스는 typed plan 경로를 standard bootstrap으로 제시한다. 개별 config와 `create_*_client()`는 CLI, 배치, 단일 서비스 시험 또는 SDK lifecycle 직접 제어에 적합한 direct API다.^[raw/articles/docmesh-py-core-api-v0-1-1.md]^[raw/articles/docmesh-py-core-api-reference-v0-4-0.md]

## 설계 관점에서 중요한 점
- 공개 API는 패키지 루트의 `__all__`로 한정되며, 구현 모듈의 비공개 심볼과 facade 전용 심볼은 신규 소비 코드의 import 대상이 아니다. `Service`, `RuntimePlan`, `HealthcheckPolicy`, `SERVICE_CATALOG`, `diagnose_services()`와 구조화된 오류 taxonomy는 typed runtime 계획과 사전 환경 진단을 명시한다.^[raw/articles/docmesh-py-core-api-reference-v0-4-0.md]
...
- v0.4.0 예제는 RuntimePlan bootstrap과 사전 진단, NATS의 지연 연결·drain, Keycloak provisioning dry-run, 선택적 MinIO bucket, aggregate 종료 오류 처리, masking/retry/logging 및 runtime preset을 함께 제시한다.^[raw/articles/docmesh-py-core-examples-v0-4-0.md]
